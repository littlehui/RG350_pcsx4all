...
static MENU gui_MainMenu = { MENU_SIZE, 0, 102, 130, (MENUITEM *)&gui_MainMenuItems };
...
static MENU gui_GameMenu = { GMENU_SIZE, 0, 102, 116, (MENUITEM *)&gui_GameMenuItems };
...
{
	port_printf(6 * 8, 10 * 5, "Skip BIOS logos at startup");
}
...
{
	port_printf(6 * 8, 10 * 5, "Analog Stick -> Arrow Keys");
}
...
{
	port_printf(6 * 8, 10 * 5, "Analog Mode");
}
...
{
	port_printf(2 * 8 - 4, 10 * 5, "Parasite Eve 2, Vandal Hearts 1/2 Fix");
}
...
static MENU gui_SettingsMenu = { SET_SIZE, 0, 56, 60, (MENUITEM *)&gui_SettingsItems };
...
static void videoscaling_hint() {
	...
	case 0:
		...
		break;
	case 1:
		port_printf(4 * 8, 70, "Software nearest-neighbour");
		break;
	}
}
...
static MENU gui_GPUSettingsMenu = { SET_GPUSIZE, 0, 56, 83, (MENUITEM *)&gui_GPUSettingsItems };
...
static MENU gui_SPUSettingsMenu = { SET_SPUSIZE, 0, 56, 83, (MENUITEM *)&gui_SPUSettingsItems };
...
static MENU gui_CheatMenu = { 0, 0, 24, 78, NULL, 13 };
...
static void ShowMenu(MENU *menu)
{
	...
	for (int i = 0; i < cnt; i++, mi++) {
		ShowMenuItem(menu->x, menu->y + i * 12, mi);
